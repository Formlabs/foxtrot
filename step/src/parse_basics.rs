...
pub fn step_decimal(input: &str) -> Res<&str, i64> {
    context("decimal", tuple((
        opt(one_of("-+")),
        many1(one_of("0123456789"))
    ))) (input).map(|(next_input, res)| (next_input, {
        let (opt_sign, digits) = res;
        let sign = match opt_sign { None => 1, Some(c) => match c { '+' => 1, '-' => -1, _=> panic!("unexpected char") }};
...
pub fn step_float(input: &str) -> Res<&str, f64> {
    context("float", tuple((
        ...
        let sign = match opt_sign { Some(_) => -1f64, None => 1f64 };
        let s: String = before_dec.into_iter().chain(vec!['.'].into_iter()).chain(after_dec.into_iter()).collect();
...
pub fn step_udecimal(input: &str) -> Res<&str, usize> {
    context("udecimal", many1(one_of("0123456789")))(input).map(|(next_input, res)| (next_input, {
...
pub fn paren_tup(input: &str) -> Res<&str, ()> {
    delimited(tag("("), separated_list0(paren_tup, take_till(|c| c == '(' || c == ')')), tag(")"))(input)
...
pub fn step_vec<'a, F: 'a, O>(inner: F) -> impl FnMut(&'a str) -> Res<&'a str, Vec<O>>
  ...
  F: FnMut(&'a str) -> Res<&'a str, O>,
{
    delimited(tag("("), separated_list0(after_ws(tag(",")), after_ws(inner)), after_ws(tag(")")))
}
...
mod tests {
    ...
    #[test]
    fn test_id_vec() {
        assert_eq!(step_vec(step_id)("(  #012345, #4556    ) ff"), Ok((" ff", vec![Id(12345), Id(4556)])));
        assert_eq!(step_vec(step_id)("(  #0123 ) ff"), Ok((" ff", vec![Id(123)])));
